...
File: `trusted-gmp/config/verifier.toml` (relative to project root)
...
- Each `EscrowEvent` includes a `chain_type` field (Mvm, Evm, Svm) set by the verifier based on which monitor discovered the event
