...
A native GMP relay service that watches for `MessageSent` events on source chains and delivers cross-chain messages by calling `deliver_message` on destination chains.

Trusted-gmp is a pure relay — invisible to clients. The coordinator is the single API surface for frontends and solvers.

## Architecture

The trusted-gmp service is infrastructure only:
...
- **Native GMP Relay**: Watches `MessageSent` events and calls `deliver_message` on destination chains
...
│   ├── native_gmp_relay/ # Core relay logic
