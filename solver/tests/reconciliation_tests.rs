...
// Verifies that Drift detected when tracker says Fulfilled but hub has no proof.
...
// Verifies that Drift detected when tracker says Created but hub already has proof.
...
// Verifies that No drift when tracker and hub agree on Fulfilled.
...
// Verifies that No drift when tracker says Created and hub has no proof yet.
...
// Verifies that Non-active states (Signed, Expired, Failed) never produce drift.
...
// Verifies that healed_state() returns the correct target state per drift variant.
...
// Verifies that heal_state_by_intent_id updates the state of the tracked intent.
...
// Verifies that heal_state_by_intent_id returns an error when the intent_id is unknown.
...
// Verifies that ReconciliationService::new() constructs successfully from a default config.
...
// Verifies that run_once() returns an empty Vec when the tracker has no intents.
...
// Verifies that run_once() skips inflow intents entirely — no hub query, no drift.
