...
//! that the `intent_id` is encoded in the payload so the validation contract can link the
...
/// which includes intent_id as a parameter so the validation contract can extract it from the
...
/// extracted by the validation contract. In contrast, EVM cannot use a contract function to transfer
...
fn generate_mvm_template(args: &Args) -> Result<()> {
    ...
    println!("  - Include intent_id in the transaction payload for on-chain validation\n");
...
/// data and are verifiable on-chain by the validation contract.
///
...
/// function signature), but they remain in the transaction data for on-chain validation. This
...
fn generate_evm_template(args: &Args) -> Result<()> {
    ...
    println!("  - Transfer tokens from your account to the recipient address");
    println!("  - Include intent_id in the transaction calldata for on-chain validation\n");
