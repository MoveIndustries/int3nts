...
mod test_helpers;
use test_helpers::{
    DUMMY_PUBLIC_KEY, DUMMY_REGISTERED_AT, DUMMY_SOLVER_ADDR_EVM, DUMMY_SOLVER_ADDR_SVM,
...
// ============================================================================
// GET_SOLVER_SVM_ADDRESS TESTS
// ============================================================================

/// Create a mock SolverRegistry resource response with SVM address in hex string format
/// This tests the case where Aptos serializes Option<vector<u8>> as {"vec": ["0xhexstring"]}
    ...
    let solver_entry = if let Some(svm_addr) = solver_connected_chain_svm_addr {
        json!({
            ...
            "value": {
                ...
                "connected_chain_svm_addr": {"vec": [svm_addr]},
                "registered_at": DUMMY_REGISTERED_AT
            }
        })
    } else {
...
/// Test that get_solver_svm_address handles array format from Aptos
/// Why: Aptos can serialize Option<vector<u8>> as {"vec": [[bytes_array]]}
/// This test verifies we correctly parse the array format for 32-byte SVM addresses
#[tokio::test]
async fn test_get_solver_svm_address_array_format() {
    ...
    let resources_response = create_solver_registry_resource_with_svm_address(
        solver_registry_addr,
        solver_addr,
        Some(solver_connected_chain_svm_addr),
    );
    ...
    let result = client
        .get_solver_svm_address(solver_addr, solver_registry_addr)
        .await;
    ...
    assert_eq!(
        address,
        Some(solver_connected_chain_svm_addr.to_string()),
        "Should return the SVM address when serialized as array format"
    );
}

/// Test that get_solver_svm_address handles hex string format from Aptos
/// Why: Aptos can serialize Option<vector<u8>> as {"vec": ["0xhexstring"]}
/// This test verifies we correctly parse the hex string format for 32-byte SVM addresses
#[tokio::test]
async fn test_get_solver_svm_address_hex_string_format() {
    ...
    let resources_response = create_solver_registry_resource_with_svm_address_hex_string(
        solver_registry_addr,
        solver_addr,
        Some(solver_connected_chain_svm_addr),
    );
    ...
    assert_eq!(
        address,
        Some(solver_connected_chain_svm_addr.to_string()),
        "Should return the SVM address when serialized as hex string format"
    );
}

/// Test that get_solver_svm_address handles leading zero mismatch
/// Why: Move strips leading zeros from addresses in type names
#[tokio::test]
async fn test_get_solver_svm_address_leading_zero_mismatch() {
    ...
    let solver_entry = json!({
        ...
        "value": {
            ...
            "connected_chain_mvm_addr": {"vec": []},
            "connected_chain_evm_addr": {"vec": []},
            "connected_chain_svm_addr": {"vec": [solver_connected_chain_svm_addr]},
            "registered_at": DUMMY_REGISTERED_AT
        }
    });
    ...
    let result = client
        .get_solver_svm_address(solver_addr, solver_registry_addr_full)
        .await;
