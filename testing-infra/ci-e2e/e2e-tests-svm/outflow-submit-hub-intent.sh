...
DESIRED_METADATA_CHAIN2="$SVM_TOKEN_HEX"

log "     Outflow configuration:"
log "       Offered metadata (hub):           $OFFERED_METADATA_HUB"
log "       Desired metadata (connected SVM): $DESIRED_METADATA_CHAIN2"
...
DRAFT_DATA=$(build_draft_data \
    ...
    "{\"chain_addr\": \"$HUB_MODULE_ADDRESS\", \"flow_type\": \"outflow\", \"connected_chain_type\": \"svm\", \"requester_addr_connected_chain\": \"$REQUESTER_CHAIN2_ADDRESS\"}")
...
log "     Offered metadata (hub): $OFFERED_METADATA_HUB"
log "     Desired metadata (connected chain): $DESIRED_METADATA_CHAIN2"
...
aptos move run --profile requester-chain1 --assume-yes \
    --function-id "0x${HUB_MODULE_ADDRESS}::fa_intent_outflow::create_outflow_intent_entry" \
    --args "address:${OFFERED_METADATA_HUB}" "u64:${OFFERED_AMOUNT}" "u64:${HUB_CHAIN_ID}" "address:${DESIRED_METADATA_CHAIN2}" "u64:${DESIRED_AMOUNT}" "u64:${CONNECTED_CHAIN_ID}" "u64:${EXPIRY_TIME}" "address:${INTENT_ID}" "address:${REQUESTER_CHAIN2_ADDRESS}" "address:${RETRIEVED_SOLVER}" "hex:${SOLVER_SIGNATURE_HEX}" >> "$LOG_FILE" 2>&1
