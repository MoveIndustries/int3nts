...
log "   - Deploying to Hub with address: $HUB_MODULE_ADDRESS"
cd move-intent-framework
aptos move publish --dev --profile intent-account-chain1 --named-addresses mvmt_intent=$HUB_MODULE_ADDRESS --assume-yes >> "$LOG_FILE" 2>&1
...
aptos move run --profile intent-account-chain1 --assume-yes \
    --function-id ${HUB_MODULE_ADDRESS}::fa_intent::initialize \
...
initialize_solver_registry "intent-account-chain1" "$HUB_MODULE_ADDRESS" "$LOG_FILE"

# Initialize intent registry (idempotent - will fail silently if already initialized)
log ""
log " Initializing intent registry..."
initialize_intent_registry "intent-account-chain1" "$HUB_MODULE_ADDRESS" "$LOG_FILE"
...
aptos move run --profile intent-account-chain1 --assume-yes \
    --function-id ${HUB_MODULE_ADDRESS}::fa_intent_outflow::initialize_verifier \
...
TEST_TOKENS_HUB_ADDRESS=$(get_profile_address "test-tokens-chain1")

log "   - Deploying USDhub with address: $TEST_TOKENS_HUB_ADDRESS"
cd testing-infra/ci-e2e/test-tokens
aptos move publish --profile test-tokens-chain1 --named-addresses test_tokens=$TEST_TOKENS_HUB_ADDRESS --assume-yes >> "$LOG_FILE" 2>&1
...
echo "TEST_TOKENS_HUB_ADDRESS=$TEST_TOKENS_HUB_ADDRESS" >> "$PROJECT_ROOT/.tmp/chain-info.env"
log "   ✅ USDhub address saved: $TEST_TOKENS_HUB_ADDRESS"
...
log "   - Minting $USDHUB_MINT_AMOUNT 10e-6.USDhub to Requester ($REQUESTER_HUB_ADDRESS)..."
aptos move run --profile test-tokens-chain1 --assume-yes \
    --function-id ${TEST_TOKENS_HUB_ADDRESS}::usdxyz::mint \
    --args address:$REQUESTER_HUB_ADDRESS u64:$USDHUB_MINT_AMOUNT >> "$LOG_FILE" 2>&1
...
log "   - Minting $USDHUB_MINT_AMOUNT 10e-6.USDhub to Solver ($SOLVER_HUB_ADDRESS)..."
aptos move run --profile test-tokens-chain1 --assume-yes \
    --function-id ${TEST_TOKENS_HUB_ADDRESS}::usdxyz::mint \
    --args address:$SOLVER_HUB_ADDRESS u64:$USDHUB_MINT_AMOUNT >> "$LOG_FILE" 2>&1
...
display_balances_hub "$TEST_TOKENS_HUB_ADDRESS"
