...
echo " Checking RPC endpoint: $BASE_RPC_URL"
RPC_RESPONSE=$(curl -s -m 10 -X POST "$BASE_RPC_URL" \
...
if [ -n "$GMP_ENDPOINT_ADDR" ] && [ -n "$ESCROW_ADDR" ]; then
    ...
    echo "      (in the [[connected_chain_evm]] Base section)"
    echo ""
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/deploy-base-mainnet-$(date +%Y%m%d-%H%M%S).log"
    {
        echo "Base Mainnet Deployment — $(date -u +%Y-%m-%dT%H:%M:%SZ)"
        echo ""
