...
use integrated_gmp::monitor::{EscrowEvent, IntentEvent};
...
/// Verifies that integrated-gmp accepts escrows where reserved_solver matches the hub intent solver.
...
async fn test_escrow_solver_address_matching_success() {
    ...
    let validation_result =
        integrated_gmp::validator::inflow_generic::validate_intent_fulfillment(
            &validator,
            &hub_intent,
            &escrow_match,
        )
...
/// Verifies that integrated-gmp rejects escrows where reserved_solver doesn't match the hub intent solver.
...
/// Verifies that integrated-gmp rejects escrows when one side has a reserved_solver and the other doesn't.
