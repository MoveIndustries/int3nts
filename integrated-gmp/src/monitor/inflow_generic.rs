...
//! 2. Integrated-GMP monitors connected chain for escrow events
//! 3. Integrated-GMP validates escrow matches hub intent requirements
//! 4. Integrated-GMP monitors hub chain for fulfillment events
//! 5. When hub fulfillment is observed, integrated-gmp generates approval signature for escrow release
...
pub async fn monitor_connected_chain(monitor: &EventMonitor) -> Result<()> {
    ...
    loop {
        ...
        tokio::time::sleep(std::time::Duration::from_millis(
            monitor.config.integrated_gmp.polling_interval_ms,
        ))
        .await;
    }
}
...
/// - **Escrow validity**: Integrated-GMP validates escrow before solver fulfills (when escrow was cached)
