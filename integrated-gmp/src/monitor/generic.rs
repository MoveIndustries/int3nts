...
/// The integrated-gmp monitors these events to track new trading opportunities
...
pub struct EscrowEvent {
    ...
    /// Note: This is set by the integrated-gmp based on which monitor discovered the event (from config),
    /// not from the event data itself, so it can be trusted for validation.
    pub chain_id: u64,
    /// Type of blockchain where this escrow is located
    /// Note: This is set by the integrated-gmp based on which monitor discovered the event,
    /// not from the event data itself, so it can be trusted for validation.
...
/// The integrated-gmp monitors these events to track when hub intents are completed,
...
/// processing them according to the integrated-gmp's validation rules. It maintains
...
impl EventMonitor {
    ...
    pub async fn new(config: &Config) -> anyhow::Result<Self> {
        ...
        let hub_client = reqwest::Client::builder()
            .timeout(std::time::Duration::from_millis(
                config.integrated_gmp.validation_timeout_ms,
            ))
    ...
    /// 2. Confirms matching escrow exists (integrated-gmp already validated escrow earlier)
    ...
    /// - Escrow validity: Integrated-GMP validates escrow before solver fulfills (future: provides signature to solver)
