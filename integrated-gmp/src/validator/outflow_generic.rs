...
/// fulfill intents. The integrated-gmp will reject transactions from unregistered or
    ...
    let registered_evm_addr = match registered_evm_addr {
        ...
        None => {
            tracing::warn!(
                ...
                Check integrated-gmp logs for detailed parsing information.",
                reserved_solver,
                hub_registry_addr
            );
            return Ok(Some(ValidationResult {
                valid: false,
                message: format!(
                    "Reserved solver '{}' is not registered in hub chain solver registry or has no connected chain EVM address. Check integrated-gmp logs for detailed parsing information.",
                    reserved_solver
                ),
                timestamp: chrono::Utc::now().timestamp() as u64,
            }));
